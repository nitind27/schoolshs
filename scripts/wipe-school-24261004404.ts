/**
 * Wipe ALL operational / dummy data for school code 24261004404.
 * Keeps: School row, SchoolSettings, SchoolSubscription, school_admin users.
 *
 * Usage: npx tsx scripts/wipe-school-24261004404.ts
 */
import { rm } from "fs/promises";
import { prisma } from "../src/lib/db";
import { getStudentUploadRoot } from "../src/lib/student-documents.server";
...
async function main() {
  ...
  await prisma.$transaction(
    async (tx) => {
      ...
      if (studentIds.length) {
        ...
        await tx.examSeatAssignment.deleteMany({ where: { studentId: { in: studentIds } } });
      ...
      if (nonAdminUserIds.length) {
        ...
        await tx.passwordChangeEvent.deleteMany({ where: { userId: { in: nonAdminUserIds } } });
  ...
  for (const id of studentIds) {
    try {
      await rm(getStudentUploadRoot(id), { recursive: true, force: true });
    } catch {
  ...
  console.log("\nDone. School record + settings/subscription preserved. Lists will show empty.");
}
