...
const HEADER_ALIASES: Record<string, ImportFieldKey> = (() => {
  ...
  const extras: [string, ImportFieldKey][] = [
    ...
    ["g.r. no", "grNumber"],
    ["g.r. number", "grNumber"],
    ["general register", "grNumber"],
    ["admission no", "grNumber"],
    ["admission number", "grNumber"],
    ["જીઆર", "grNumber"],
    ["જી.આર.", "grNumber"],
    ["પ્રવેશ નંબર", "grNumber"],
    ["ssc seat prefix", "sscSeatPrefix"],
    ["10th seat prefix", "sscSeatPrefix"],
    ["ssc seat", "sscSeatNumber"],
    ["10th seat number", "sscSeatNumber"],
    ["hsc seat prefix", "hscSeatPrefix"],
    ["12th seat prefix", "hscSeatPrefix"],
    ["hsc seat", "hscSeatNumber"],
    ["12th seat number", "hscSeatNumber"],
    ["id card valid", "idCardValidUpto"],
    ["10th %", "percentage10th"],
    ["ssc %", "percentage10th"],
    ["12th %", "percentage12th"],
    ["hsc %", "percentage12th"],
    ...
    ["આધાર", "aadhaarNumber"],
    ["મોબાઇલ", "mobileNumber"],
    ["first name gujarati", "firstNameGu"],
    ["middle name gujarati", "middleNameGu"],
    ["surname gujarati", "surnameGu"],
    ["aadhaar name gujarati", "aadhaarNameGu"],
    ["mother name gujarati", "motherNameGu"],
    ["father name gujarati", "fatherNameGu"],
    ["guardian name gujarati", "guardianNameGu"],
    ["id card valid upto", "idCardValidUpto"],
  ];
...
export const IMPORT_FIELD_GROUPS: { id: string; title: string; fields: ImportFieldKey[] }[] = [
  ...
  },
  {
    ...
    fields: ["grNumber", "standard", "section", "rollNumber", "childUid"],
  },
  {
    id: "family",
    title: "Family",
  ...
  },
  {
    id: "hostel",
    title: "Hostel",
    fields: ["isHosteler", "hostelType", "hostelName"],
  },
  {
    id: "bank",
    title: "Bank",
...
export function resolveImportTemplateFields(requested: string[]): ImportFieldKey[] {
  ...
  const source = picked.length > 0 ? new Set<ImportFieldKey>(picked) : null;
  const ordered: ImportFieldKey[] = [];
  for (const k of CSV_HEADERS) {
    if (REQUIRED_IMPORT_FIELDS.includes(k) || !source || source.has(k)) {
      ordered.push(k);
    }
  }
  return ordered;
