...
export function IdCardShareLinkManager({ classId, standard, section, academicYear, classes }: Props) {
  ...
  const resolveUrl = (link: ShareLink) =>
    link.shareUrl || `${appUrl || window.location.origin}/m/id-cards/${link.slug}`;
      ...
      <CardHeader>
        ...
        {appUrl && (
          <p className="text-[11px] text-violet-600 mt-1 font-medium">
            {t("idCardShare.appUrlNote")}: <span className="font-mono">{appUrl}</span>
          </p>
        )}
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {lastUrl && (
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2.5 space-y-2">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-emerald-700">{t("idCardShare.linkUrl")}</p>
            <div className="flex items-center gap-2">
              <a href={lastUrl} target="_blank" rel="noopener noreferrer" className="flex-1 truncate font-mono text-xs text-emerald-900 hover:underline">
                {lastUrl}
              </a>
              ...
              <a
                ...
                className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-emerald-700 hover:bg-emerald-100"
              >
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {links.map((link) => {
              const url = resolveUrl(link);
              return (
                <div key={link.id} className="rounded-xl border border-slate-200 bg-white p-3 space-y-2">
                  <div className="flex flex-col sm:flex-row sm:items-start gap-2">
                    ...
                    <div className="flex shrink-0 gap-1">
                      <Button size="sm" variant="outline" onClick={() => copyUrl(link)} title={t("idCardShare.copyLink")}>
                        {copiedId === link.id ? <span className="text-xs">{t("idCardShare.copied")}</span> : <Copy className="h-3.5 w-3.5" />}
                      </Button>
                      <a
                        href={url}
                        target="_blank"
                        rel="noopener noreferrer"
                        title={t("idCardShare.openLink")}
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="block truncate rounded-md bg-slate-50 border border-slate-100 px-2.5 py-1.5 font-mono text-[11px] text-violet-700 hover:bg-violet-50 hover:border-violet-200 transition-colors"
                  >
                    {url}
                  </a>
                </div>
