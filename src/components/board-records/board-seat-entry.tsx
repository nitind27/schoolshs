...
function toEntryRow(s: EntryStudent, standard: "10" | "12"): EntryRow {
  return {
    ...
    grNumber: s.grNumber || "",
    seatPrefix:
      standard === "10" ? s.sscSeatPrefix || "A" : s.hscSeatPrefix || "B",
    seatNumber:
      standard === "10" ? s.sscSeatNumber || "" : s.hscSeatNumber || "",
    percentage: String(
      standard === "10"
        ? s.percentage10th > 0
          ? s.percentage10th
          : ""
        : s.percentage12th && s.percentage12th > 0
          ? s.percentage12th
          : "",
    ),
...
export function BoardSeatEntry({
  ...
  useEffect(() => {
    if (!requestedClassId || classId || !classes.length) return;
    const requested = classes.find((item) => item.id === requestedClassId);
    if (!requested) return;
    const timer = window.setTimeout(() => {
      if (isHsc) {
        const streamGroup = hscStreams.find((group) =>
          group.classes.some((item) => item.id === requestedClassId),
        );
        if (streamGroup) setSelectedStream(streamGroup.stream);
      }
      setClassId(requestedClassId);
    }, 0);
    return () => window.clearTimeout(timer);
  }, [classId, classes, hscStreams, isHsc, requestedClassId]);
  ...
  const loadClass = useCallback(async (id: string) => {
    ...
    try {
      ...
      setRows(
        (data.students || []).map((s: EntryStudent) => toEntryRow(s, std)),
      );
    } catch (e) {
      setMessage({
        type: "err",
  ...
  useEffect(() => {
    if (!classId) return;
    const timer = window.setTimeout(() => void loadClass(classId), 0);
    return () => window.clearTimeout(timer);
  }, [classId, loadClass]);
  ...
  const updateRow = (idx: number, field: keyof EntryRow, value: string) => {
    setRows((prev) => {
      ...
      if (field === "seatNumber")
        v = value.replace(/\D/g, "").slice(0, seatDigitLen(standard));
      next[idx] = { ...next[idx], [field]: v };
      return next;
    });
  };

  const applyPrefixToAll = (prefix: string) => {
  ...
  const saveAll = async () => {
    ...
    try {
      ...
      setMessage({
        type: "ok",
        text: t("boardRecords.entrySaved", { count: data.updated }),
      });
      onSaved?.();
      loadClass(classId);
      fetch("/api/board-records/overview")
  ...
  const uploadExcel = async (file: File) => {
    ...
    try {
      const res = await fetch("/api/board-records/upload", {
        method: "POST",
        body: fd,
      });
      const data = await res.json();
      ...
      setMessage({
        type: "ok",
        text: t("boardRecords.excelImported", { count: data.updated }),
      });
      onSaved?.();
      loadClass(classId);
    } catch (e) {
  ...
  const filledSeats = rows.filter(
    (r) => r.seatNumber.length === digitLen,
  ).length;
  const displayStep = isHsc ? step : Math.max(1, step - 1);

  const steps = isHsc
    ? [
        t("boardRecords.stepStream"),
        t("boardRecords.stepDivision"),
        t("boardRecords.stepEntry"),
      ]
    : [t("boardRecords.stepDivision"), t("boardRecords.stepEntry")];

  const pctCol =
    standard === "10" ? t("boardRecords.colPct10") : t("boardRecords.colPct12");
      ...
      <div className="rounded-2xl bg-white border border-slate-200 p-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {step > 1 && (
            <button
              onClick={goBack}
              className="ml-auto text-xs font-semibold text-slate-500 hover:text-blue-600"
            >
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-sm opacity-90 mt-1">
                      {sg.classes.length} {t("boardRecords.divisions")} ·{" "}
                      {sg.studentCount} {t("boardRecords.students")}
                    </p>
                  </button>
                );
              })}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {(isHsc ? streamClasses : sscDivisions).map((cls) => (
                <button
                  key={cls.id}
                  onClick={() =>
                    selectClass(cls.id, isHsc ? selectedStream : undefined)
                  }
                ...
                >
                  ...
                  <div className="mt-2 flex gap-2 text-[11px]">
                    ...
                    <span className="bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full font-bold">
                      {cls.seatsFilled}/{cls.studentCount}{" "}
                      {t("boardRecords.seatsShort")}
                    </span>
                  </div>
                  {cls.classTeacher && (
          ...
          <div className="rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-200 p-5 space-y-4">
            <div className="flex items-start justify-between gap-3 flex-wrap">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <label className="flex items-center gap-2 h-10 px-4 rounded-xl bg-violet-600 text-white text-sm font-semibold hover:bg-violet-700 cursor-pointer">
                  ...
                  {uploading
                    ? t("boardRecords.uploading")
                    : t("boardRecords.uploadExcel")}
                  <input
                    type="file"
                    accept=".xlsx,.xls"
                    className="hidden"
                ...
                >
                  ...
                  {saving
                    ? t("boardRecords.saving")
                    : t("boardRecords.saveAll")}
                </button>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3 text-sm">
              <span className="flex items-center gap-1.5 bg-white rounded-lg px-3 py-1.5 border border-blue-200">
                ...
                <strong>{classInfo?.studentCount}</strong>{" "}
                {t("boardRecords.students")}
              </span>
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
                <div className="min-w-[220px]">
                  <label className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-violet-800">
                    {t("boardRecords.applyPrefixAll")}
                  </label>
                  <select
                    value={bulkPrefix}
                    onChange={(event) => applyPrefixToAll(event.target.value)}
                    className="h-11 w-full rounded-xl border-2 border-violet-300 bg-violet-50 px-3 text-base font-black text-violet-800 outline-none focus:border-violet-500"
                  >
                    <option value="">{t("boardRecords.selectAlphabet")}</option>
                    {seatPrefixes.map((prefix) => (
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    {t("boardRecords.gsebStorageHint", {
                      example: standard === "12" ? "P 654545" : "P 6545454",
                    })}
                  </p>
                </div>
              </div>
          ...
          {loading ? (
            ...
            <div className="rounded-2xl bg-white border border-amber-200 bg-amber-50 p-12 text-center">
              ...
              <p className="font-semibold text-slate-800">
                {t("boardRecords.noStudentsInClass")}
              </p>
            </div>
          ) : (
            <div className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
              <div className="px-5 py-3 border-b bg-slate-50 flex items-center justify-between">
                <p className="text-sm font-bold text-slate-800">
                  {t("boardRecords.seatEntryTable")}
                </p>
                <p className="text-[11px] text-slate-500">
                  {t(
                    standard === "12"
                      ? "boardRecords.seatFormatHintHsc"
                      : "boardRecords.seatFormatHint",
                  )}
                </p>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b bg-blue-600">
                      {[
                        t("boardRecords.colRoll"),
                        t("boardRecords.colStudent"),
                        t("boardRecords.colGr"),
                        t("boardRecords.colSeatPrefix"),
                        t(
                          standard === "12"
                            ? "boardRecords.colSeatNoHsc"
                            : "boardRecords.colSeatNo",
                        ),
                        t("boardRecords.colFullSeat"),
                        pctCol,
                        t("boardRecords.colYear"),
                      ].map((h) => (
                        <th
                          key={h}
                          className="px-3 py-3 text-left text-[10px] font-bold text-white uppercase whitespace-nowrap"
                        >
                          {h}
                        </th>
                      ))}
                    ...
                    {rows.map((row, idx) => {
                      ...
                      return (
                        <tr key={row.studentId} className="hover:bg-blue-50/30">
                          <td className="px-3 py-2 font-semibold text-slate-600 w-14">
                            {row.roll || idx + 1}
                          </td>
                          ...
                          <td className="px-3 py-2 w-20">
                            <select
                              ...
                              onChange={(e) =>
                                updateRow(idx, "seatPrefix", e.target.value)
                              }
                          ...
                          <td className="px-3 py-2 w-32">
                            <input
                              ...
                              inputMode="numeric"
                              maxLength={digitLen}
                              placeholder={isHsc ? "123456" : "1234567"}
                              value={row.seatNumber}
                              onChange={(e) =>
                                updateRow(idx, "seatNumber", e.target.value)
                              }
                          ...
                          <td className="px-3 py-2 w-24">
                            <input
                              ...
                              value={row.examYear}
                              onChange={(e) =>
                                updateRow(idx, "examYear", e.target.value)
                              }
  ...
  return (
    <div className="rounded-2xl bg-white border border-slate-200 p-12 text-center">
      ...
      <p className="font-semibold text-slate-700">
        {t("boardRecords.noClassesForStd", { std: standard })}
      </p>
      <p className="text-sm text-slate-500 mt-1">
        {t("boardRecords.createClassFirst")}
      </p>
      {standard === "12" && (
