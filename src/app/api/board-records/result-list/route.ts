...
import {
  ...
  mergeBoardResultJson,
} from "@/lib/board-records/result-list-config";
import {
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error(e);
    return NextResponse.json(
      { error: "Failed to load board result list" },
      { status: 500 },
    );
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    for (const row of rows) {
      ...
      const grade = row.grade || (pct != null ? gsebGrade(pct).label : null);
      const resultText =
        row.result ||
        (pct != null
