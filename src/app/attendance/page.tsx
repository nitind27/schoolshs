...
function AttendanceContent() {
  ...
  const classLabel = `${meta.standard}-${meta.section}`;
  ...
  const downloadExport = async (format: "pdf" | "xlsx") => {
    ...
    try {
      ...
      a.href = url;
      a.download = `attendance_${classLabel}_${yearNum}-${String(monthNum).padStart(2, "0")}.${format === "pdf" ? "pdf" : "xlsx"}`;
      a.click();
      ...
      {loading ? (
        ...
        <>
          <AttendanceViewToolbar
            ...
            onDownload={filters.classId ? downloadExport : undefined}
            downloading={exporting}
          />

          <div className="flex flex-wrap items-center gap-3 text-sm text-slate-600">
            <span className="font-semibold text-slate-900">
              {t("attendance.classLabel")}: {classLabel}
            </span>
            <span>·</span>
